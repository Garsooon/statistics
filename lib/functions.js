...
async function getNations() {
    let nations = await request(process.env.nationsURL);
    return nations['nations'] && !nations['error'] ? nations['nations'] : false;
}

// get a nation's information
async function getNation(uuid) {
    let nation = await request(process.env.nationURL+uuid);
    return nation && !nation['error'] ? nation : false;
}
...
async function isPlayerOnline(username) {
    ...
    if(!server || !server['players']) return { online: false };
